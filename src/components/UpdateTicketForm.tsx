  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //var avail = ;
        // setAvailableNames(accommodationData.find((c) => c.name === response1.data.ticket.accommodation_type)?.accommodations!);
        // setAvailableRooms(availableNames.find((c) => c.name === response1.data.ticket.accommodation_name)?.rooms!);
        // setAvailableLocations(availableRooms.find((c) => c.name === response1.data.ticket.accommodation_room_number)?.locations!);
        setSelectedType(accommodationData.find((c) => c.name === response1.data.ticket.accommodation_type)!);
        setSelectedName(accommodationData.find((c) => c.name === response1.data.ticket.accommodation_type)?.accommodations.find((a) => a.name === response1.data.ticket.accommodation_name)!);
        setSelectedRoom(accommodationData.find((c) => c.name === response1.data.ticket.accommodation_type)?.accommodations.find((a) => a.name === response1.data.ticket.accommodation_name)?.rooms.find((b) => b.name === response1.data.ticket.accommodation_room_number)!);
        setSelectedLocation(accommodationData.find((c) => c.name === response1.data.ticket.accommodation_type)?.accommodations.find((a) => a.name === response1.data.ticket.accommodation_name)?.rooms.find((b) => b.name === response1.data.ticket.accommodation_room_number)?.locations.find((l) => l.name === response1.data.ticket.accommodation_specific_location)!);
      } catch (err) {
        setLoading(false);
      } finally {
        setLoading(false);
